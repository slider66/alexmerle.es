...
const SERVICIOS: Servicio[] = [
  ...
  {
    id: "google-business",
    label: "Google Business optimizado",
    desc: "Ficha de Google, fotos, categorías, reseñas, horarios.",
    price: 150,
  ...
  },
  {
    id: "auditoria-seo",
    label: "Auditoría SEO",
    desc: "Análisis técnico + informe de mejoras concretas y accionables.",
    price: 150,
    category: "Presencia online",
  },
  {
    ...
    desc: "Página enfocada en convertir visitas en contactos o encargos.",
    price: 150,
    category: "Presencia online",
  },
  {
    id: "web-completa",
    label: "Web completa",
    desc: "Web profesional con SEO local, Google Maps, diseño responsive y contacto directo.",
    price: 449,
    category: "Presencia online",
  },
  {
    id: "web-reservas",
    label: "Web + Sistema de reservas",
    desc: "Todo lo anterior más agenda online con confirmación automática.",
    price: 599,
    category: "Presencia online",
  },
  ...
  {
    id: "buzon",
    label: "Buzón centralizado",
    desc: "Unifica email, WhatsApp y consultas en un solo punto de gestión.",
    price: 150,
    category: "Comunicación y captación",
  },
  {
    id: "whatsapp-auto",
    label: "Respuestas automáticas WhatsApp",
    desc: "Responde las preguntas frecuentes fuera de horario y en festivos.",
    price: 150,
    category: "Comunicación y captación",
  },
  {
    id: "agenda",
    label: "Agenda online",
    desc: "El cliente reserva solo, sin llamar. Confirmación automática.",
    price: 150,
    category: "Comunicación y captación",
  },
  {
    id: "citas-clinica",
    label: "Sistema de citas (clínicas)",
    desc: "Reservas con confirmación y recordatorio. Cumple normativa RGPD.",
    price: 299,
    category: "Comunicación y captación",
  },
  // Automatización
  {
    id: "carta-digital",
    label: "Carta digital + encargos",
    desc: "Carta en el móvil y sistema de encargos por WhatsApp con confirmación automática.",
    price: 150,
  ...
  },
  {
    ...
    desc: "Mapeo de los 3 procesos que más tiempo roban + automatización básica.",
    price: 150,
    category: "Automatización",
  },
];
...
function calcularCombos(selected: Set<string>): ComboSuggestion[] {
  ...
  const tienePresenciaBasica =
    ...
    selected.has("google-business") &&
    !selected.has("web-completa") &&
    !selected.has("web-reservas");
...
export function DossierPresupuestador() {
  ...
  const toggle = (id: string) => {
    setSelected((prev) => {
      ...
      if (next.has(id)) {
        ...
        if (id === "web-reservas") {
          next.delete("web-completa");
          next.delete("agenda");
        }
        if (id === "web-completa") {
          next.delete("web-reservas");
          next.delete("landing");
        }
        next.add(id);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const lineas = selectedItems.map((s) => `  • ${s.label}: ${s.price} € + IVA`).join("\n");
...
Si quieres hablar, escríbeme a hola@webpc.es o por WhatsApp: +34 600 367 217
    ...
    <div className="space-y-6">
      ...
      <div className="print-hide space-y-6">
                ...
                <div className="divide-y divide-white/5">
                  {items.map((s) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`w-5 h-5 rounded-md border flex items-center justify-center shrink-0 mt-0.5 transition-colors ${isSel ? "bg-brand-teal border-brand-teal" : "border-white/20 bg-transparent"}`}
                        >
                          {isSel && <Check size={11} className="text-black" strokeWidth={3} />}
                        </span>
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm font-bold leading-snug ${isSel ? "text-white" : "text-white/65"}`}
                          >
                            {s.label}
                          </p>
                          <p className="text-[11px] text-white/35 font-medium mt-0.5 leading-relaxed">
                            {s.desc}
                          </p>
                        </div>
                        <span
                          className={`text-sm font-black shrink-0 tabular-nums ${isSel ? "text-brand-teal" : "text-white/40"}`}
                        >
                          {s.price} €
                        </span>
                      </button>
        ...
        {combos.map((combo) => (
          <div
            key={combo.id}
            className="flex items-start gap-4 p-5 rounded-xl border border-brand-blue/25 bg-brand-blue/5"
          >
      ...
      {selected.size > 0 && (
        <div
          className="glass-card p-7 bg-[#080808]/60 border-white/10"
          style={{ borderRadius: "1rem" }}
        >
          <div className="space-y-2 mb-6">
            {selectedItems.map((s) => (
              <div key={s.id} className="flex items-center justify-between gap-4">
                ...
                <span className="text-sm font-bold text-white/80 tabular-nums shrink-0">
                  {s.price} €
                </span>
              </div>
            ))}
          ...
          <div className="border-t border-white/10 pt-4 space-y-1 mb-6">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <span className="text-2xl font-black tabular-nums text-brand-teal">
                {Math.round(total * 1.21)} €
              </span>
            </div>
          </div>
              ...
              <form onSubmit={handleSubmit} className="mt-5 space-y-4 border-t border-white/8 pt-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      placeholder={
                        action === "email" ? "tu@email.com" : "tu@email.com o 600 000 000"
                      }
                      value={contact}
                ...
                {formState === "error" && (
                  <p className="text-red-400/70 text-xs font-medium">
                    Algo ha fallado. Prueba de nuevo o escríbeme directamente a hola@webpc.es.
                  </p>
                )}
              ...
              <div className="mt-5 border-t border-white/8 pt-5 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-white/80">
                    {action === "download"
                      ? "¡Listo! Abriendo el PDF…"
                      : "¡Enviado! Revisa tu bandeja de entrada."}
                  </p>
                  <p className="text-[11px] text-white/40 font-medium mt-0.5">
                    {action === "email"
                      ? "Si no aparece en unos minutos, revisa la carpeta de spam. También puedes escribirme directamente a hola@webpc.es."
          ...
          <p className="text-white/20 text-[10px] font-medium mt-4 print-muted">
            Orientativo. El presupuesto final se cierra tras el diagnóstico inicial. Pago 50% al
            arrancar, 50% a la entrega.
          </p>
        </div>
      )}
